%% This is used to fit hyperparameters for sigma from RBF KRR cost function.
clear all
close all
clc
...
for i = 1:length(sigmaList)
    ...
    path = sprintf('hypresults/%s_%.1f_%s_ksigma%.2e',dataSet,r_value,costFunction,hyp.k_sigma);
    mkdir(path);
